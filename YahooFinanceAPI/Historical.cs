    ...
    public class Historical
    {
        ...
        public static async Task<List<HistoryPrice>> GetPriceAsync(string symbol, DateTime start, DateTime end)
        {
            try
            {
                ...
                if (csvData != null) return await ParsePriceAsync(csvData, symbol).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        private static async Task<List<HistoryPrice>> ParsePriceAsync(string csvData, string Symbol)
        {
